...
const POSTSCORE_COLLECTION_NAME = 'postScore';
const POSTSCORE_COLLECTION_SCHEMA = Joi.object({
...
const checkPostwithPostId = async (post_id) => {
  return await GET_DB()
    .collection(POSTSCORE_COLLECTION_NAME)
...
const addUserToUserList = async (post_id, userLike_id) => {
  return await GET_DB()
    .collection(POSTSCORE_COLLECTION_NAME)
    .updateOne({ post_id: post_id }, { $push: { userList: userLike_id } });
};
...
const removeUserFromUserList = async (post_id, userLike_id) => {
  return await GET_DB()
    .collection(POSTSCORE_COLLECTION_NAME)
    .updateOne({ post_id: post_id }, { $pull: { userList: userLike_id } });
};
...
const createNew = async (post_id, userLike_id) => {
  try {
    ...
    if (!postScore) {
      ...
      const validPostScore = await POSTSCORE_COLLECTION_SCHEMA.validateAsync(
        newPostScore
      );
      return await GET_DB()
        .collection(POSTSCORE_COLLECTION_NAME)
        .insertOne(validPostScore);
    } else {
